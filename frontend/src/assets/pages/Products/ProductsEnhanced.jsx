...
const Products = () => {
  ...
  const applyFilters = () => {
    ...
    switch (sortBy) {
      ...
      case 'rating':
        filtered.sort((a, b) => {
          const ratingA = Number(b.rating?.average || b.rating || 0);
          const ratingB = Number(a.rating?.average || a.rating || 0);
          return ratingA - ratingB;
        });
        break;
      case 'popular':
        filtered.sort((a, b) => {
          const reviewsA = Number(b.rating?.count || b.reviews || 0);
          const reviewsB = Number(a.rating?.count || a.reviews || 0);
...
const ProductCard = ({ product, onAddToCart, isFavorite, onToggleFavorite, quantity, onUpdateQuantity }) => {
  ...
  const rating = Number(product.rating?.average || product.rating || 0);
  const reviewCount = Number(product.rating?.count || product.reviews || 0);
      ...
      <div className="p-5">
        ...
        <div className="mb-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-500">({reviewCount || 0} reviews)</span>
          </div>
        </div>
